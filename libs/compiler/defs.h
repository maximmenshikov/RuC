...
{
	MIPS_IC_bltz   = 1,     // bltz rs, label       if rs < 0                   bgez rs, label  if rs >= 0
	MIPS_IC_jump   = 2,     // j label
	MIPS_IC_jal    = 3,     // jal label
	MIPS_IC_beq    = 4,     // beq rs, rt, label    if rs == rt
	MIPS_IC_bne    = 5,     // bne rs, rt, label    if rs != rt
	MIPS_IC_blez   = 6,     // blez rs, label       if rs <= 0
	MIPS_IC_bgtz   = 7,     // bgtz rs, label       if rs > 0
	MIPS_IC_addi   = 8,     // addi rt, rs, imm     rt = rs + SignImm
	MIPS_IC_addiu  = 9,     // addiu rt, rs, imm    rt = rs + SignImm           no overflow
	MIPS_IC_slti   = 10,    // slti rt, rs, imm     rt = rs < SignImm ? 1 : 0
	MIPS_IC_sltiu  = 11,    // sltiu rt, rs, imm    rt = rs < SignImm ? 1 : 0   unsigned
	MIPS_IC_andi   = 12,    // andi rt, rs, imm     rt = rs & ZeroImm
	MIPS_IC_ori    = 13,    // ori rt, rs, imm      rt = rs | ZeroImm
	MIPS_IC_xori   = 14,    // xori rt, rs, imm     rt = rs ^ ZeroImm
	MIPS_IC_lui    = 15,    // lui rt, imm          rt = {imm, 16'b0}
	MIPS_IC_mul    = 28,    // mul rd, rs, rt       rd = rs * rt
	MIPS_IC_lw     = 35,    // lw rt, imm(rs)       rt = [Address]
	MIPS_IC_sw     = 43,    // sw rt, imm(rs)       [Address] = rt
	MIPS_IC_sll    = 60,    // sll rd, rt, shamt    rd = rt << shsmt
	MIPS_IC_srl    = 62,    // srl rd, rt, shamt    rd = rt >> shsmt
	MIPS_IC_sra    = 63,    // sra rd, rt, shamt    rd = rt >> shsmt            arithmetic
	MIPS_IC_sllv   = 64,    // sllv rd, rt, rs      rd = rt << [rs]4:0
	MIPS_IC_srlv   = 66,    // srlv rd, rt, rs      rd = rt >> [rs]4:0
	MIPS_IC_srav   = 67,    // srav rd, rt, rs      rd = rt >> [rs]4:0          arithmetic
	MIPS_IC_jr     = 68,    // jr rs                PC = rs
	MIPS_IC_jalr   = 69,    // jalr rs
	MIPS_IC_add    = 92,    // add rd, rs, rt       rd = rs + rt
	MIPS_IC_addu   = 93,    // addu rd, rs, rt      rd = rs + rt                unsigned
	MIPS_IC_sub    = 94,    // sub rd, rs, rt       rd = rs - rt
	MIPS_IC_subu   = 95,    // subu rd, rs, rt      rd = rs - rt                unsigned
	MIPS_IC_and    = 96,    // and rd, rs, rt       rd = ts & rt
	MIPS_IC_or     = 97,    // or rd, rs, rt        rd = ts | rt
	MIPS_IC_xor    = 98,    // xor rd, rs, rt       rd = ts ^ rt
	MIPS_IC_nor    = 99,    // nor rd, rs, rt       rd = ~(rs | rt)
	MIPS_IC_slt    = 102,   // slt rd, rs, rt       rd = rs < rt ? 1 : 0
	MIPS_IC_sltu   = 103,   // sltu rd, rs, rt      rd = rs < rt ? 1 : 0        unsigned
	MIPS_IC_li     = 115,   // li rt, imm(32)       это псевдокоманда (lui + ori)
} mips_ic;